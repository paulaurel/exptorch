...
class LinearNet(nn.Module):
    ...
    def training_step(self, batch, loss_criterion):
        x, y = batch
        x_pred = self.forward(x)
        return loss_criterion(x_pred, y)
...
class MLP(nn.Module):
    ...
    def training_step(self, batch, batch_idx, loss_criterion):
        ...
        import ipdb
        ipdb.set_trace()
        return loss_criterion(y_pred, y)

    def configure_optimizers(self, optimizer: torch.optim, **kwargs):
        return optimizer(self.net.parameters(), **kwargs)

    # def configure_optimizers(self, optimizers: Struct, optimizer_params: Sturct):
    #     opt_gen = optimizers.gen(self.gen, **optimizer_params.lr_gen)
    #     opt_desc = optimizers.gen(self.desc, **optimizer_params.lr_desc)
    #     return [opt_gen, opt_desc]
